...
@Component({
  selector: "aeliusmd-add-parameters",
  templateUrl: "./add-parameters.component.html",
  styleUrls: [
    "./add-parameters.component.css",
    "./add-parameters.component.scss",
  ],
})
export class AddMacroFieldParameterComponent implements OnInit, OnChanges {
  /* #region declarations */
  // _msgs: Message[] = [];
  _recordStatus = RecordStatus;
  _recordStatusDropdown = EnumToDropdown.transform(RecordStatus);
  _pageTitle: string = "Appointment Resource Shift";
  _pageSubTitle: string = "/ Add";
  ...
  @Output() macroFieldParameterAddedEventHandler = new EventEmitter<object>();
  ...
  onAdd() {
    ...
    let addObject = this._form.get('parameter').value;

    let hasParameter = false;

    //this.parameterList.filter()

    if (!hasParameter) {
      this.macroFieldParameterAddedEventHandler.emit(addObject);
  ...
  close() {
    if (this._form.dirty == true) {
      this.confirmationService.confirm({
        ...
        reject: () => {
          this.macroFieldParameterAddedEventHandler.emit();
        },
      });
    } else {
      this.macroFieldParameterAddedEventHandler.emit();
